...
async function main() {
  try {
    console.log('[SECURITY_FIX] Guaranteeing full 43-menu Enterprise RBAC link across all tenant schemas...');

    const tenants = await prisma.$queryRawUnsafe(`SELECT db_name, name, plan FROM nexus.tenants`);
    console.log('Tenants in system:', tenants);

    for (const t of tenants) {
      const schema = t.db_name.toLowerCase();
      console.log(`\n=================== Processing: ${t.name} (Schema: ${schema}) ===================`);

      try {
        // 1. Ensure table structure & constraints exist
        ...
        // 2. Insert all master menus into rbac_menus
        await prisma.$executeRawUnsafe(`
        ...
        // 3. Link ALL menus to ADMIN role (without failing on composite key)
        const linkedCount = await prisma.$executeRawUnsafe(`
          ...
          WHERE r.name = 'ADMIN'
          AND NOT EXISTS (
            SELECT 1 FROM "${schema}".rbac_role_menus rm WHERE rm.role_id = r.id AND rm.menu_id = m.id
          );
        `);

        console.log(`Successfully linked ${linkedCount} missing menus to ADMIN role in ${schema}`);

        // Verify total linked menus for ADMIN
        const countRes = await prisma.$queryRawUnsafe(`
          ...
          WHERE r.name = 'ADMIN'
        `);
        console.log(`Total ADMIN menus in ${schema}: ${countRes[0].total}`);
    ...
    console.log('\n[SECURITY_FIX] Successfully updated all tenant schemas.');

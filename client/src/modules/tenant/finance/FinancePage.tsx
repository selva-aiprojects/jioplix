      ...
      <main className="main-content">
        ...
        <div style={{ padding: "20px 24px" }}>
          ...
          {loading ? (
            ...
            <>
              {tab === "dashboard" && (
                <div className="grid-responsive" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "16px", marginBottom: "20px" }}>
                  <div className="stat-card"><div style={{ fontSize: "12px", color: "#64748b" }}>Packages</div><div style={{ fontSize: "24px", fontWeight: 900 }}>{packages.length}</div></div>
                  <div className="stat-card"><div style={{ fontSize: "12px", color: "#64748b" }}>Surgery Cases</div><div style={{ fontSize: "24px", fontWeight: 900 }}>{cases.length}</div></div>
                  <div className="stat-card"><div style={{ fontSize: "12px", color: "#64748b" }}>Open Advances</div><div style={{ fontSize: "24px", fontWeight: 900 }}>{advances.filter((a: any) => a.status === "OPEN").length}</div></div>
                  <div className="stat-card"><div style={{ fontSize: "12px", color: "#64748b" }}>Pending Refunds</div><div style={{ fontSize: "24px", fontWeight: 900 }}>{refunds.filter((x: any) => x.status === "PENDING").length}</div></div>
                  <div className="stat-card"><div style={{ fontSize: "12px", color: "#64748b" }}>Pending Write-offs</div><div style={{ fontSize: "24px", fontWeight: 900 }}>{writeoffs.filter((x: any) => x.status === "PENDING").length}</div></div>
                </div>
              )}
